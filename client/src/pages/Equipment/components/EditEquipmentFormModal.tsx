// src/pages/Equipment/components/EditEquipmentFormModal.tsx
...
interface EditEquipmentFormModalProps {
...
const EditEquipmentFormModal: FC<EditEquipmentFormModalProps> = ({
  equipment,
  onEquipmentUpdated,
  refreshKey,
  isOpen,
  ...
  const handleUpdateEquipment = async (e: FormEvent) => {
    try {
      ...
      if (res.status === 200) {
        onEquipmentUpdated(
          res.data.message || "Equipment successfully updated",
        );
        refreshKey();
        onClose();
      }
  ...
  useEffect(() => {
    if (isOpen && equipment) {
      ...
      setExistingImage(equipment.image || null);
      setImage(null);
      ...
      setAreaId(equipment.area_id?.toString() || "");
      setQuantity(equipment.quantity || 1);
      setAvailableQuantity(equipment.available_quantity || 1);
      setUnit(equipment.unit || "pcs");
      setPurchaseDate(equipment.purchase_date || "");
      setPurchaseCost(equipment.purchase_cost?.toString() || "");
        ...
        <div className="grid grid-cols-2 gap-4 border-b border-[#c9a84c]/20 pb-4 max-h-[64vh] overflow-y-auto pr-2">
          ...
          <div className="col-span-2 rounded-lg border border-[#c9a84c]/15 bg-[#0E1A3A]/30 p-4">
            <UploadInput
              ...
              value={image}
              onChange={setImage}
              onRemoveExistingImageUrl={() => setExistingImage(null)}
